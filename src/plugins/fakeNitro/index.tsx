...
function CannotEmbedNoticeModal({
    modalProps,
    resolve,
}: {
    ...
    return (
        <ConfirmModal
            ...
            checkboxProps={{
                ...
                onChange: (checked) =>
                    (s.disableEmbedPermissionCheck = checked),
            }}
        />
    );
}
...
function showCannotEmbedNotice() {
    return new Promise<boolean>((resolve) => {
        openModal((props) => (
            <CannotEmbedNoticeModal modalProps={props} resolve={resolve} />
        ));
    });
}
...
export default definePlugin({
    ...
    patches: [
        ...
        },
        {
            ...
            replacement: [
                ...
                },
                {
                    ...
                    replace: (check) =>
                        `(${check}||${IS_BYPASSEABLE_INTENTION})`,
                },
                {
            ...
            replacement: [
                {
                    ...
                    replace: (_, props) =>
                        `$self.handleProtoChange(${props}.userSettingsProto,${props}.user);`,
                },
                {
        ...
        },
        {
            ...
            replacement: {
                ...
                replace: (_, reactNode) =>
                    `$self.addFakeNotice(${FakeNoticeType.Emoji},${reactNode},!!arguments[0]?.fakeNitroNode?.fake)`,
            },
        },
    ...
    patchFakeNitroStickers(stickers: Array<any>, message: Message) {
        ...
        for (const item of itemsToMaybePush) {
            ...
            if (gifMatch) {
                ...
                const stickerName =
                    StickersStore.getStickerById(gifMatch[1])?.name ??
                    "FakeNitroSticker";
                stickers.push({
                    format_type: 2,
                    id: gifMatch[1],
                ...
                case "image": {
                    ...
                    if (
                        ...
                        !contentItems.some(
                            (item) =>
                                item === url ||
                                item.match(hyperLinkRegex)?.[1] === url,
                        )
                    )
                        return false;
                    ...
                    if (settings.store.transformStickers) {
                        ...
                        if (gifMatch) {
                            // There is no way to differentiate a regular gif attachment from a fake nitro animated sticker, so we check if the StickersStore contains the id of the fake sticker
                            if (StickersStore.getStickerById(gifMatch[1]))
                                return true;
                        }
                    }
    ...
    async sendAnimatedSticker(
        ...
        const file = new File(
            [gif.bytesView() as Uint8Array<ArrayBuffer>],
            `${stickerId}.gif`,
            { type: "image/gif" },
        );
    ...
    start() {
        ...
        this.preSend = addMessagePreSendListener(
            async (channelId, messageObj, options) => {
                const { guildId } = this;
                ...
                stickerBypass: {
                    ...
                    const sticker = StickersStore.getStickerById(
                        options.stickerIds?.[0]!,
                    );
        ...
        this.preEdit = addMessagePreEditListener(
            async (channelId, __, messageObj) => {
